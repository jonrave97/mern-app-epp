...
export default function Aside({ sidebarOpen }: { sidebarOpen: boolean }) {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 z-40 w-64 h-full transition-transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0`}
      aria-label="Sidebar"
    >
        ...
        <ul className="space-y-2 font-medium">
          ...
          <li className='px-3'>
            ...
            >
                  ...
                  <svg className={`w-5 h-5 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m19 9-7 7-7-7"/></svg>
            </button>
            <ul className={`${dropdownOpen ? '' : 'hidden'} py-2 space-y-2`}>
                  <li>
